...
void task5_5::tests_::extra_tests()
{
	{
		//what is the size?
		vector< int > int_vector;
		BOOST_CHECK_EQUAL( sizeof( int_vector ), 24ul );
	}

	{
		//here memoryleack detected
		vector< int > iv;
		for( int i = 0; i < 10; ++i )
			iv.push_back( i );
	...
	{
		//a bit harder assignment operator test
		vector< int > iv;
		for( int i = 0; i < 10; ++i )
			iv.push_back( i );
		...
		iv[ 0 ] = 1234;
		BOOST_CHECK_EQUAL( iv[ 0 ], 1234 );
		BOOST_CHECK_EQUAL( iv_copy[ 0 ], 0 );
	}

	{
		//a bit harder const this[] test
		const vector< int > iv;
