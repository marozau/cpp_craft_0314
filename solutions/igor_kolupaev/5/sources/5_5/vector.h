...
namespace task5_5
{
	...
	class vector
	{
		...
		template< typename T >
		void copy_items( T* dest, const T* src, size_t count )
		{
		...
		void expand( size_t new_capacity )
		{
			...
			copy_items( new_items, items_, this->size() );
		...
		void copy_vector( vector<T>* dest, const vector<T>* src )
		{
			...
			copy_items( this->items_, src->items_, src->size_ );
	...
	{
		return const_cast<T*>( this->items_ );
	}
	template< typename T >
	typename vector< T >::const_iterator vector< T >::end() const
	{
		return const_cast<T*>( items_ + size() );
	}
}
