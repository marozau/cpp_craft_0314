...
void binary_reader::tests_::test_stock_data()
{
	...
	}
	{
		...
		stock_data sd( in );
		BOOST_CHECK_EQUAL( memcmp( sd.stock_name(), "AAPL", 4 ), 0 );
		BOOST_CHECK_EQUAL( memcmp( sd.date_time(), "20130101", 8 ), 0 );
		BOOST_CHECK_EQUAL( fabs( sd.price() - 100.0 ) < eps, true );
		BOOST_CHECK_EQUAL( fabs( sd.vwap() - 101.1 ) < eps, true );
		BOOST_CHECK_EQUAL( fabs( sd.f1() - 1.1 ) < eps, true );
		BOOST_CHECK_EQUAL( fabs( sd.t1() - 2.1 ) < eps, true );
		BOOST_CHECK_EQUAL( fabs( sd.f2() - 2.2 ) < eps, true );
		BOOST_CHECK_EQUAL( fabs( sd.f3() - 3.3 ) < eps, true );
		BOOST_CHECK_EQUAL( fabs( sd.f4() - 4.4 ) < eps, true );
		BOOST_CHECK_EQUAL( sd.volume(), 20000 );
	}
